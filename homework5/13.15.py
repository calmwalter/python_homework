...
class AddressBook:
    def __init__(self):
        ...
        Button(frame4, text="First",command=self.processFirst).grid(row=1, column=2)
        Button(frame4, text="Next",command=self.processNext).grid(row=1, column=3)
        Button(frame4, text="Previous",command=self.processPrevious).grid(row=1, column=4)
        Button(frame4, text="Last",command=self.processLast).grid(row=1, column=5)
        Button(frame4, text="Update",command=self.processUpdate).grid(row=1, column=6)
        ...
        frame5=Frame(window)
        frame5.pack()
        self.label1=Label(frame5,text="Current adress index:%d/number of adress:%d"%(self.current+1,len(self.addressList)))
    ...
    def processAdd(self):
        address = Address(self.nameVar.get(), self.streetVar.get(
        ), self.cityVar.get(), self.stateVar.get(), self.zipVar.get())
        self.addressList.append(address)
        self.saveAddress()
        self.updateLabel()
    def updateLabel(self):
        self.label1["text"]="Current adress index:%d/number of adress:%d"%(self.current+1,len(self.addressList))
    ...
    def processUpdate(self):
        ...
        self.addressList[self.current]=address
        outfile = open("address.dat", "wb")
        pickle.dump(self.addressList, outfile)
        tkinter.messagebox.showinfo("Address updated", "The address has been updated")
        outfile.close()
    def setAddress(self):
